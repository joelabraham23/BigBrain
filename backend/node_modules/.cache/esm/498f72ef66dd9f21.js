let fs,jwt,AsyncLock,InputError,AccessError,quizQuestionPublicReturn,quizQuestionGetCorrectAnswers,quizQuestionGetDuration;_66d‍.x([["save",()=>save],["reset",()=>reset],["userLock",()=>userLock],["quizLock",()=>quizLock],["sessionLock",()=>sessionLock],["getEmailFromAuthorization",()=>getEmailFromAuthorization],["login",()=>login],["logout",()=>logout],["register",()=>register],["assertOwnsQuiz",()=>assertOwnsQuiz],["getQuizzesFromAdmin",()=>getQuizzesFromAdmin],["addQuiz",()=>addQuiz],["getQuiz",()=>getQuiz],["updateQuiz",()=>updateQuiz],["removeQuiz",()=>removeQuiz],["startQuiz",()=>startQuiz],["advanceQuiz",()=>advanceQuiz],["endQuiz",()=>endQuiz],["sessionStatus",()=>sessionStatus],["assertOwnsSession",()=>assertOwnsSession],["sessionResults",()=>sessionResults],["playerJoin",()=>playerJoin],["hasStarted",()=>hasStarted],["getQuestion",()=>getQuestion],["getAnswers",()=>getAnswers],["submitAnswers",()=>submitAnswers],["getResults",()=>getResults]]);_66d‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_66d‍.w("jsonwebtoken",[["default",["jwt"],function(v){jwt=v}]]);_66d‍.w("async-lock",[["default",["AsyncLock"],function(v){AsyncLock=v}]]);_66d‍.w("./error",[["InputError",["InputError"],function(v){InputError=v}],["AccessError",["AccessError"],function(v){AccessError=v}]]);_66d‍.w("./custom",[["quizQuestionPublicReturn",["quizQuestionPublicReturn"],function(v){quizQuestionPublicReturn=v}],["quizQuestionGetCorrectAnswers",["quizQuestionGetCorrectAnswers"],function(v){quizQuestionGetCorrectAnswers=v}],["quizQuestionGetDuration",["quizQuestionGetDuration"],function(v){quizQuestionGetDuration=v}]]);
  ...
  new Promise((resolve, reject) => {
    lock.acquire('saveData', () => {
      try {
       ...
       const save = () => update(admins, quizzes, sessions);
       const reset = () => {
  update({}, {}, {});
  admins = {};
  quizzes = {};
...
const newPlayerId = _ => generateId(Object.keys(sessions).map(s => Object.keys(sessions[s].players)));
  ...
  lock.acquire('userAuthLock', callback(resolve, reject));
       ...
       const quizLock = callback => new Promise((resolve, reject) => {
  lock.acquire('quizMutateLock', callback(resolve, reject));
});

       const sessionLock = callback => new Promise((resolve, reject) => {
...
const newQuizPayload = (name, owner) => ({
  ...
  createdAt: new Date().toISOString(),
       ...
       const assertOwnsQuiz = (email, quizId) => quizLock((resolve, reject) => {
  if (!(quizId in quizzes)) {
    ...
    resolve();
       ...
       const getQuizzesFromAdmin = email => quizLock((resolve, reject) => {
  resolve(Object.keys(quizzes).filter(key => quizzes[key].owner === email).map(key => ({
    ...
    oldSessions: getInactiveSessionsIdFromQuizId(key),
       ...
       const addQuiz = (name, email) => quizLock((resolve, reject) => {
  if (name === undefined) {
    ...
    resolve(newId);
       ...
       const getQuiz = quizId => quizLock((resolve, reject) => {
  resolve({
    ...quizzes[quizId],
    ...
    oldSessions: getInactiveSessionsIdFromQuizId(quizId),
       ...
       const updateQuiz = (quizId, questions, name, thumbnail) => quizLock((resolve, reject) => {
  ...
  resolve();
       ...
       const removeQuiz = quizId => quizLock((resolve, reject) => {
  delete quizzes[quizId];
  resolve();
});

       const startQuiz = quizId => quizLock((resolve, reject) => {
  if (quizHasActiveSession(quizId)) {
    ...
    resolve(id);
       ...
       const advanceQuiz = quizId => quizLock((resolve, reject) => {
  ...
  if (!session.active) {
    ...
    resolve(session.position);
       ...
       const endQuiz = quizId => quizLock((resolve, reject) => {
  const session = getActiveSessionFromQuizIdThrow(quizId);
...
const newPlayerPayload = (name, numQuestions) => ({
  ...
  await assertOwnsQuiz(email, sessions[sessionId].quizId);
       ...
       const sessionResults = sessionId => sessionLock((resolve, reject) => {
  const session = sessions[sessionId];
  if (session.active) {
    ...
    resolve(Object.keys(session.players).map(pid => session.players[pid]));
       ...
       const playerJoin = (name, sessionId) => sessionLock((resolve, reject) => {
  if (name === undefined) {
    ...
    if (session.position >= 0) {
      ...
      resolve(parseInt(id, 10));
       ...
       const hasStarted = playerId => sessionLock((resolve, reject) => {
  ...
  if (session.isoTimeLastQuestionStarted !== null) {
    ...
    resolve(false);
       ...
       const getQuestion = playerId => sessionLock((resolve, reject) => {
  const session = getActiveSessionFromSessionId(sessionIdFromPlayerId(playerId));
  if (session.position === -1) {
    ...
    resolve({
      ...
      isoTimeLastQuestionStarted: session.isoTimeLastQuestionStarted,
       ...
       const getAnswers = playerId => sessionLock((resolve, reject) => {
  const session = getActiveSessionFromSessionId(sessionIdFromPlayerId(playerId));
  if (session.position === -1) {
    return reject(new InputError('Session has not started yet'));
    ...
    resolve(quizQuestionGetCorrectAnswers(session.questions[session.position]));
       ...
       const submitAnswers = (playerId, answerList) => sessionLock((resolve, reject) => {
  if (answerList === undefined || answerList.length === 0) {
    ...
    if (session.position === -1) {
      ...
      resolve();
       ...
       const getResults = playerId => sessionLock((resolve, reject) => {
